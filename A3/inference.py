...
#def gibbs_sampler(tree, factors, query, vocab, num_itr = 3000, burn_in = 2000):

def gibbs_predict(imgs, mrf, factors):
	chars = "d,o,i,r,a,h,t,n,s,e".split(",")
	char_inv = {char : val for (val, char) in enumerate(chars)}
	begin_time = time.time()
	predictions, counts, samples = gibbs_sampler(mrf, factors, {}, {var: chars for var in mrf})
	...
	for var in xrange(l1):
		word1 += chars[predictions[var]]

	for var in xrange(l1, l1+l2):
		word2 += chars[predictions[var]]

	marginals = {}

	for var in xrange(l1+l2):
		curr_sum = 0.0
		probabs = []
		for i in xrange(len(chars)):
			curr_probab = math.log(1.0 + counts[var][i])
			probabs.append(curr_probab)
			curr_sum += 1.0 + counts[var][i]

		curr_sum = math.log(curr_sum)
		probabs = [probab - curr_sum for probab in probabs]
		marginals[var] = probabs

	return [word1, word2], marginals, time.time() - begin_time


def predict(imgs, model, tables, alg, run_for, useMAP=False):
	assert(alg in ['mp', 'lbp', 'gibbs'])
	chars = "d,o,i,r,a,h,t,n,s,e".split(",")
	char_inv = {char : val for (val, char) in enumerate(chars)}
	...
	if alg == 'gibbs':
		return gibbs_predict(imgs, mrf, factors)

...
class Factor:
	...
	def reduce(self, queryDict):
		'''
			queryDict is a dictionary of var : assignment pairs
		'''
		if (len(queryDict) == 0):
			return copy.deepcopy(self)

		V = set(queryDict.keys())
		phi = Factor()
		phi.scope = self.scope - V
		phi.numPossibleValues = {}
		phi.table = []
		...
		for var in (self.scope):
			if (var in queryDict):
				stride += self.strides[var]*queryDict[var]

		for i in xrange(productTableSz):
			assignment = phi._assig(i)
			base_idx = self._idx(assignment)
			idx = base_idx + stride
			phi.table.append(self.table[idx])
